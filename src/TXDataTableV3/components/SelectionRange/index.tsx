...
export const SelectionRange = withState({
  states: [ 'onSelectionChange', 'fetchConfig', 'fetchedData', 'rowkey', 'columns' ],
    ...
    const handleCopyAndPasteClipboard = (e) => {
      ...
      if (e.code === 'KeyV' && (e.ctrlKey || e.metaKey) && selectedCells !== null) {
        readClipboardText().then(text => {
          ...
          const updatedData = updateDataSourceFromExcelWithoutMutation(data, selectedCells, rowkey, columns, text);

          if (fetchConfig) {
            setGlobalStateByKey('fetchedData', { ...fetchedData, data: updatedData });
          } else {
            setGlobalStateByKey('localData', updatedData);
          }
        });
      }
    };
  ...
  React.useImperativeHandle(selectionRangeRef, () => ({
